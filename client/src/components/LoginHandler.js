...
function LoginHandler({isRegistered,setIsRegistered,setIsLogin}){
    ...
    async function submitHandler(event){
        ...
        let data=await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/v1/login`,{
            method:"POST",
            credentials:"include",
    ...
    return(
        <div className="w-full h-full my-6">
             <form ref={form} className='w-3/5 max-md:w-full h-1/2 flex flex-col gap-y-5 mx-auto shadow-lg '>
                <h1 className='text-center text-2xl font-semibold my-2 font-sans '>Log In</h1>
